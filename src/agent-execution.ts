...
export class FairAgentScheduler {
  ...
  #inherit(parent: ScheduledNode | undefined, options: ScheduledAgentOptions): Readonly<ScheduledAgentOptions> {
    const unknown = Object.keys(options).find((key) => !["label", "parentBreadcrumb", "cwd", "tools", "isolation", "worktreeOwner", "model", "thinking", "role", "schema", "retries", "timeoutMs"].includes(key));
