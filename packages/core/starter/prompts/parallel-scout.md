...
Otherwise split the question into 2-3 independent areas (for example: the feature's entry points, the data layer it touches, the tests that cover it). Launch a named workflow whose script fans out one `agent(taskPrompt, { role: "scout" })` per area with `parallel(...)`, putting its question and context in `taskPrompt`, then merge the keyed briefs yourself into one map: relevant files, how the flow works end to end, patterns to reuse, and open questions.
