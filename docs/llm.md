...
Trajectory keeps the original native, independently collapsible Gantt timeline for phase, agent, subagent, and tool timing. It does not use Mermaid for the timeline. A separate local Mermaid flowchart appears immediately below the Gantt in a workflow run overview. The overview contains a workflow root, observed phase occurrences from valid persisted `phaseHistory` (or a flat workflow grouping when that evidence is unavailable), structural-path scope containment, agents with state, and per-agent tool activity aggregated by name with status and count. A valid persisted `parentId` may add an agent-to-agent relationship; `structuralPath` and `parentBreadcrumb` are grouping/containment evidence, not proof of dependency order. When exact workflow structure is absent, bounded, or malformed, the overview degrades to flat containment, partial/running status, or an explicit no-data state rather than inventing pipeline, parallel, or timestamp-derived dependencies.

Opening a workflow agent or standalone subagent shows an independent detail topology of tool invocations. Each invocation keeps its source call event index and, when available, its result event index. A non-empty tool-call ID is correlated only when that ID occurs exactly once among calls and exactly once among results; duplicate, missing, unknown, or partial IDs never select an unrelated result. If no unique result exists, the topology may conservatively use the assistant source event only when the invocation is unambiguous and its existing inspector content is meaningful. Persisted `agent.toolCalls` summaries without transcript events remain noninteractive. Calls emitted in the same assistant entry branch from the agent root; later assistant tool-call entries are connected to the preceding transcript turn where the transcript provides that structure. Timestamps are used for the timeline and duration display only and never establish dependency edges. Activating an invocation with the mouse, Enter, or Space selects the exact event, reveals its row within the events scroller without moving the page, and opens the same right-side inspector and Input/Output/Schema/Timing panes as direct row selection; the node exposes a descriptive button name and selected state. The overview and detail graphs have separate collapse keys, signatures, render generations, DOM targets, zoom, and viewport state, so either graph can update or change theme without replacing the native Gantt or the other graph while its own zoom and scroll position persist.

Both topology levels use the locally bundled Mermaid runtime with strict security, deterministic IDs, sanitized bounded labels, the current theme, and static-export support. Static reports inline the local runtime rather than depending on a remote script. Topology zoom is true intrinsic SVG zoom: 100% uses readable viewBox dimensions rather than fitted width, 50%-200% is available in 25-point steps, zoom changes both node/text size and the scrollable canvas, reset returns to true 100%, ordinary wheel input remains panning, and Ctrl/Cmd-wheel changes zoom. In the run view, Gantt, overview topology, AGENTS, and logs are independently collapsible; old `traj-run-layout` objects are read with defaults for newly added keys. AGENTS columns have persisted `traj-agent-columns` widths, sensible clamped minima/maxima, horizontal overflow, mouse resizing, keyboard arrow resizing, and double-click/keyboard reset. Standalone subagents remain first-class publisher entities in a sibling `SUBAGENTS` section; their native Gantt lane and detail topology are not fabricated workflow-run data. Running subagents expose Stop and Steer, while failed or stopped subagents expose Retry; retry creates a new subagent ID and follows that ID in the UI. Pause, resume, and checkpoint controls do not apply to subagents.
