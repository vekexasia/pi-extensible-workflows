...
The run view adds a collapsible `TOPOLOGY` tree below the agents grid: workflow, phases, structural scopes, agents nested under their parent agent, and one chip per tool name with its call count (red when any call failed); clicking an agent opens its transcript. The agent view shows a tool-call flow strip above the events: one column per assistant turn, one chip per tool call (red when the result failed, `…` when no result is recorded yet); clicking a chip selects and reveals the matching transcript event.
