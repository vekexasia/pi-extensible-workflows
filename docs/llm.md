...
Live Trajectory provides two separate native topology views. The workflow overview is an evidence-backed tree assembled from recorded phases, scopes, parent relationships, and agent/tool status; missing or malformed relationships are shown conservatively rather than inferred. An agent view provides a bounded, ordered turn sequence with same-turn tool fan-out. Labels are sanitized and output is bounded for safe deterministic rendering. Invocation controls are keyboard-accessible and, when a call has one unambiguous transcript row, select that row and its inspector entry. Static HTML reports retain the native Gantt, events, and inspector content but omit topology and do not require any additional runtime or network resource.
